...
**학습 목표.** 의존성 있는 bean을 등록했을 때, 엔진이 생성자 파라미터의 **타입**을 보고 이미 registry에 있는 다른 bean을 찾아 주입(inject)한 후 인스턴스를 만드는 과정을 구현한다.
...
- `ConstructorResolver.java.tmpl` — 핵심. `getDeclaredConstructors()[0]`, 파라미터 타입별 registry lookup, `newInstance(args)` 호출
- `README.md`
...
- `testChainedDependency` — `A(B b), B(C c)` → C → B → A 순서 검증 (A.b.c != null)
- `testUnresolvedDependencyThrows` — B가 등록되지 않은 상태에서 A 요청 시 예외
...
기존 pattern 유지: **graded 4문항(MCQ 2 + short_answer 2, 각 weight 25) + reflection 1문항, passingScore 60.**

### 4.1 `quiz-di-03`

**Graded**
- q1 (MCQ): "Spring이 singleton bean을 재생성하지 않도록 보장하는 자료구조는?" → ConcurrentHashMap 기반 singleton map
- q2 (short): "BeanDefinition은 metadata, singleton instance는 런타임 생성물. 둘을 분리하는 이유를 한 단어로: ___" → lazy (또는 lifecycle, 둘 다 alias)
- q3 (MCQ): "같은 이름으로 `getBean` 두 번 호출 시 Spring의 기본 동작은?" → same instance reused
...
`QUIZ_SPECS` inline map에 `spring.di.03`, `spring.di.04` 엔트리 추가. YAML (`packages/pack-spring-core/quiz/di-03-*.yaml`, `di-04-*.yaml`) 도 authoring source 아카이브로 함께 추가 (현 구조는 YAML이 source-of-truth document이고 실제 loading은 inline map으로 한다는 주석에 따름).
